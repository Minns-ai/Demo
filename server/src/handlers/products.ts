  ...
  switch (intent.intent) {
    case 'product_info': {
      ...
      if (productId) {
        ...
        if (!product) return { response: `Product ${productId} not found.` };
        return {
      ...
      if (query) {
        ...
        if (results.length === 0) return { response: `I couldn't find any products matching "${query}".` };
    ...
    case 'recommend': {
      const category = intent.slots.category;
