...
export function handleReturns(intent: ParsedSidecarIntent, customerId: string): HandlerResult {
  ...
  switch (intent.intent) {
    case 'initiate_return': {
      ...
      if (!order) return { response: `I couldn't find order ${orderId}. Could you check the order number?` };
      if (order.status === 'returned') return { response: `Order **${orderId}** has already been returned.` };
      const reason = intent.slots.reason || 'not specified';
      return {
    ...
    case 'exchange': {
      ...
      const order = findOrder(orderId);
      if (!order) return { response: `Order ${orderId} not found.` };
      return {
